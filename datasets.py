...
def getages(mn, mx, mu, noOfUsers, sigma = 30):
	my_list = np.random.normal(mu, sigma, noOfUsers)
	new_list = []
	for x in my_list:
		a = x
		while (mn <= a <= mx) == False:
			a = np.random.normal(mu, sigma)
		new_list.append(math.floor(a))
	return new_list

def getProbabilities(mu, sigma, noOfUsers):
	my_list = np.random.normal(mu, sigma, noOfUsers)
	new_list = []
	for x in my_list:
		a = x
...
def getCountries(noOfUsers = 2000):
	...
	for i in range(0, j):
		countryData = countryData + copy.copy(countries)
	return countryData

def getUserNames(noOfUsers):
	accounts = []
	for i in range(0, noOfUsers):
		accounts.append(id(random.choice(range(6,12))))
	return accounts

def getTwitterIDs(noOfUsers):
	accounts = []
	for i in range(0, noOfUsers):
		accounts.append('@'+id(random.choice(range(5,10))))
	return accounts
