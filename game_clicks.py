...
def writeGameClicksForTeam(teamID, team, time):
	...
	for row in gameClicks:
		appendFile.write("%s, clickid=%s,  userid=%s, usersessionid=%s, isHit=%s, teamID=%s\n," %
			(row[0].strftime(global_vars.timestamp_format), row[1], row[2], row[3], row[4], teamID))
	appendFile.close()
...
def calculateHitsRequired(teamID, team):
	...
	if teamID not in global_vars.teamLevelTracker:
		...
		if currentLevel <= 1:
			currentLevel = 2
		#required hits
		reqTotalHits = ((currentLevel+2) * (currentLevel+2) - 1)
